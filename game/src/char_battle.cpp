...
bool CHARACTER::Attack(LPCHARACTER pkVictim, BYTE bType)
{
	...
	if (IsPC())
	{
		...
		// @fixme58
		if (bType == 0 && DISTANCE_APPROX(pkVictim->GetX() - GetX(), pkVictim->GetY() - GetY()) > 250)
			return false;
...
void CHARACTER::ItemDropPenalty(LPCHARACTER pkKiller)
{
	...
	// @fixme - arenada agresifken nesne dusmez
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	if (pkKiller && pkKiller->IsPC() && this->GetRaceNum() == 2493)
	{
		...
		pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DRAGON_RANKING_UPDATE"));
	}
#endif
	...
	if (pkKiller && pkKiller->IsPC() && IsPC() && GetDesc()->GetHostName() != pkKiller->GetDesc()->GetHostName())
	{
		if (CThreeWayWar::instance().IsThreeWayWarMapIndex(GetMapIndex()))
		{
			if (pkKiller->GetNationalPoint() + GetNationalPointByVictimLevel() > NATIONAL_POINT_WAIT_TIME)
				pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("NATIONAL_POINT_LIMIT_WARNING"));
			else
			{
				pkKiller->PointChange(POINT_NATIONAL_POINT, GetNationalPointByVictimLevel());
				pkKiller->EffectPacket(SE_NATIONAL_POINT);
			}
		}
		else
		{
			if (time(0) < pkKiller->GetLastNationalPointReward() + NATIONAL_POINT_WAIT_TIME)
			{
				pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("NATIONAL_POINT_WAIT%d_WARNING"), NATIONAL_POINT_WAIT_TIME);
			}
			else
			{
				if (pkKiller->GetNationalPoint() + GetNationalPointByVictimLevel() > NATIONAL_POINT_WAIT_TIME)
				...
				else
				{
					pkKiller->PointChange(POINT_NATIONAL_POINT, GetNationalPointByVictimLevel());
					pkKiller->EffectPacket(SE_NATIONAL_POINT);
					pkKiller->SetLastNationalPointReward(time(0));
				}
			}
		}
	}
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
	...
	else if (type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE)
	{
		...
		if (pAttacker)
		{
			...
			// HP 스틸 - Yeni HP Calma Hesaplamasi
			...
			// SP 스틸 - Yeni SP Calma Hesaplamasi
			...
			{
				int i = ((iCurHP >= 0) ? MIN(dam, iCurHP):dam) * pAttacker->GetPoint(POINT_HIT_HP_RECOVERY) / 100; //@fixme11
			...
			{
				int i = ((iCurHP >= 0) ? MIN(dam, iCurHP):dam) * pAttacker->GetPoint(POINT_HIT_SP_RECOVERY) / 100; //@fixme11
	...
	// 마나쉴드(흑신수호)
	//
	if (IsAffectFlag(AFF_MANASHIELD)) // @fixme31
	{
...
#elif defined(WJ_WOLFMAN_CHARACTER) && defined(USE_MOB_BLEEDING_AS_POISON)
		else if (type == DAMAGE_TYPE_BLEEDING)
			damageFlag = DAMAGE_POISON;
#endif
		else
	...
	if (pAttacker && dam > 0 && IsNPC())
	{
		...
		if(type != DAMAGE_TYPE_POISON) // @fixme32
...
#ifdef ENABLE_NEWSTUFF
	m_iRealAlignment = MINMAX((g_iMaxRankPoints*-10), m_iRealAlignment + iAmount, (g_iMaxRankPoints*10));
#else
...
void CHARACTER::UpdateAggrPointEx(LPCHARACTER pAttacker, EDamageType type, int dam, CHARACTER::TBattleInfo & info)
{
	...
	// Zehir yedikten sonra tekrar dogunca canavarlar otomatik saldirmaz
	if (type != DAMAGE_TYPE_POISON)
