...
namespace MeleyLair
{
	...
	void CMgrMap::PrepareStart()
	{
		pkMainNPC = Spawn((DWORD)(NPC_VNUM), 135, 179, 8), pkGate = Spawn((DWORD)(GATE_VNUM), 129, 175, 5);
	}
	
	void CMgrMap::PrepareDestroy()
	{
		if (pkStatue1)
			pkStatue1->Dead();
		if (pkStatue2)
	...
	void CMgrMap::Start()
	{
		if (!pkSectreeMap) {
			...
			m_bStart = true;
			pkBoss = Spawn((DWORD)(BOSS_VNUM), 130, 77, 1), pkStatue1 = Spawn((DWORD)(STATUE_VNUM), 123, 137, 5, true), pkStatue2 = Spawn((DWORD)(STATUE_VNUM), 123, 124, 5, true), pkStatue3 = Spawn((DWORD)(STATUE_VNUM), 136, 123, 5, true), pkStatue4 = Spawn((DWORD)(STATUE_VNUM), 137, 137, 5, true);
		}
	}
	...
	void CMgrMap::EndDungeon(bool bSuccess, bool bGiveBack, bool bDestroy)
	{
		...
		if (pkGuild)
		{
			...
			else
			{
				...
				{
					snprintf(szBuf1, sizeof(szBuf1), LC_TEXT("<Guild> The time limit to fight %s has ended."), "Meley");
					snprintf(szBuf2, sizeof(szBuf2), ".");					
				}
				#endif
			}
	...
	bool CMgr::IsAlreadyStart(LPCHARACTER pkChar, CGuild* pkGuild)
	{
		if ((!pkChar) || (!pkGuild))
			return false;
		...
		CMgrMap* pMap = m_RegGuilds.find(pkGuild->GetID())->second;
		if (pMap)
			return pMap->IsAlreadyStart();
	}
	
	void CMgr::End(LPCHARACTER pkChar, CGuild* pkGuild)
	{
		if ((!pkChar) || (!pkGuild))
			return;
