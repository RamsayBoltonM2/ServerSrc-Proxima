...
void CPVPManager::Insert(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	if (pkChr->IsBlockMode(BLOCK_PVP))
	{
		pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("PKCHR_BLOCK_PVP"));
		return;
	}
	
	if (pkVictim->IsBlockMode(BLOCK_PVP))
	{
		pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("PKVICTIM_BLOCK_PVP"));
		return;
	}
	...
	if ((pkPVP = Find(kPVP.m_dwCRC)))
	{
		...
		if (pkPVP->Agree(pkChr->GetPlayerID()))
		{
			...
			pkChr->EffectPacket(SE_PVP_BEGIN1);
			pkVictim->EffectPacket(SE_PVP_BEGIN2);
#endif			
		}
		return;
	}
	...
	while (it2 != it->second.end())
	{
		...
		if (dwCompanionPID == dwKillerPID)
		{
			if (pkPVP->IsFight())
			{
				...
				if (pkVictim)
					pkVictim->EffectPacket(SE_PVP_WIN);
#endif
				found = true;
				break;
			}
...
bool CPVPManager::CanAttack(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	{
		int iIsMount = __MountAtkSkill(pkChr->GetMountVnum(), false);
		switch (iIsMount)
		{
			...
			default:
				if (test_server)
					sys_log(0, "CanUseSkill: Mount can't attack. vnum(%u) type(%d)", pkChr->GetMountVnum(), iIsMount);
				return false;
				break;
		}
	}
