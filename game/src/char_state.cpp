...
void CHARACTER::__StateIdle_Monster()
{
	...
	if (!no_wander && !IS_SET(m_pointsInstant.dwAIFlag, AIFLAG_NOMOVE))
	{
		if (!number(0, 6))
		{
			...
			if (test_server) // @fixme - g_test_server->test_server
			{
				if (number(0, 100) < 60)
					SetNowWalking(false);
				else
	...
	if (1.0f == fRate)
	{
		...
		else
		{
			...
			else
			{
				...
				if (IsNewMount())
					m_dwStateDuration = PASSES_PER_SEC(0.5);
				else
					m_dwStateDuration = PASSES_PER_SEC(number(1, 3));
			}
		}
...
{
	float	START_FOLLOW_DISTANCE = 163.0f;		// 이 거리 이상 떨어지면 쫓아가기 시작함
	float	START_RUN_DISTANCE = 575.0f;		// 이 거리 이상 떨어지면 뛰어서 쫓아감.
