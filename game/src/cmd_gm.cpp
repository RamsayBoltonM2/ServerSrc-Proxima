...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent)
	{
		...
		if (!pkChr->IsOfflineShopNPC() && pkChr->GetRaceNum() != 30000 && pkChr->GetRaceNum() != 30002 && pkChr->GetRaceNum() != 30003 && pkChr->GetRaceNum() != 30004 && pkChr->IsNPC() && !pkChr->IsPet() && !pkChr->IsNewPet() && pkChr->GetRider() == NULL)
#elif defined(WJ_OFFLINESHOP_SYSTEM) and !defined(WJ_GROWTH_PET_SYSTEM)
		if (!pkChr->IsOfflineShopNPC() && pkChr->GetRaceNum() != 30000 && pkChr->GetRaceNum() != 30002 && pkChr->GetRaceNum() != 30003 && pkChr->GetRaceNum() != 30004 && pkChr->IsNPC() && !pkChr->IsPet() && pkChr->GetRider() == NULL)
#elif !defined(WJ_OFFLINESHOP_SYSTEM) and defined(WJ_GROWTH_PET_SYSTEM)
...
ACMD(do_item_purge)
{
	...
	for (i = 0; i < INVENTORY_AND_EQUIP_SLOT_MAX; ++i)
	{
		if (i > INVENTORY_MAX_NUM && i < INVENTORY_MAX_NUM + WEAR_MAX_NUM)
			continue;
...
#ifdef WJ_EXTENDED_SHOP_SYSTEM
		case DoSetTypes::COIN:	// dragon coin
			{
				int coin = 0;
				str_to_number(coin, arg3);
				tch->SetDragonCoin(tch->GetDragonCoin() + coin);
			}
#endif
		...
		case DoSetTypes::CHEQUE:	// cheque
			{
				...
				DBManager::instance().SendChequeLog(CHEQUE_LOG_MISC, 3, cheque);
				tch->PointChange(POINT_CHEQUE, cheque, true);
			}
			break;
#endif
#ifdef WJ_NATIONAL_POINT_SYSTEM
		case DoSetTypes::NATIONAL_POINT:	// np
			{
				long national_point = 0;
				str_to_number(national_point, arg3);
				if (tch->GetNationalPoint() + national_point < NATIONAL_POINT_MAX)
					tch->PointChange(POINT_NATIONAL_POINT, national_point, true);
			}
			break;
#endif
...
ACMD (do_attr_full_set)
{
	...
	case JOB_WOLFMAN:
#endif
		{
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute( 0, APPLY_CAST_SPEED, 100);
				item->SetForceAttribute( 1, APPLY_CRITICAL_PCT, 100);
				item->SetForceAttribute( 2, APPLY_PENETRATE_PCT, 100);
				item->SetForceAttribute( 3, APPLY_NORMAL_HIT_DAMAGE_BONUS, 60);
...
ACMD(do_kick_offline_shop)
{
	...
	if (pMap)
	{
		...
		if (offlineShop.bIsDestroyed)
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_PLAYER_SHOP_DESTROY"), szName);
		}
		else
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_PLAYER_SHOP_DONT_DESTROY"), szName);		
	}
}

ACMD(do_open_offline_shop)
{
	bool bRet = ch->HasOfflineShop();
	long iMapIndex = 0;
	int iTime = 0;
	...
	if (bRet)
	{
		...
		iMapIndex = COfflineShopManager::instance().GetMapIndex(ch);
		iTime = COfflineShopManager::instance().GetLeftTime(ch)+time(0);
	}
	else
	{
	...
	char szResult[256];
	snprintf(szResult, sizeof(szResult), "OfflineShopGui %d %d %d", bRet, iMapIndex, iTime);
	ch->ChatPacket(CHAT_TYPE_COMMAND, szResult);
}
#endif
...
ACMD(do_weapon_rarity)
{
	...
	int lWeaponRarity = atoi(arg1);
	LPITEM weapon = ch->GetWear(WEAR_WEAPON);
	if (weapon && IS_SET(weapon->GetFlag(), ITEM_FLAG_RARE_ABILITY))
	{
		weapon->SetForceAttribute(6, 0, weapon->GetAttributeValue(6) + lWeaponRarity);
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("EVOLOTION_POINT_UPDATE"));
	}
}
#endif
