...
namespace quest
{
	...
	bool CQuestManager::Initialize()
	{
		...
		m_mapEventName.insert(TEventNameMap::value_type("receive", QUEST_RECEIVE_EVENT));
#endif
...
#ifdef WJ_QUEST_RECEIVE_SYSTEM
	void CQuestManager::Receive(unsigned int pc, const char* cmd, DWORD quest_index)
	{
		PC* pPC = GetPC(pc);
		if (!pPC)
		{
	...
	void CQuestManager::Kill(unsigned int pc, unsigned int npc)
	{
		...
		if ((pPC = GetPC(pc)))
		{
			...
			*/
			// kill call script
			if (npc >= MAIN_RACE_MAX_NUM) //@fixme30
				m_mapNPC[npc].OnKill(*pPC); //@warme3
			...
			if (leader)
			{
				...
				if (npc >= MAIN_RACE_MAX_NUM) //@fixme30
					m_mapNPC[npc].OnPartyKill(*GetPC(leader->GetPlayerID())); //@warme3
	...
	{
		StopAllRunningQuests(); // reload q crash fix
		lua_close(L);
		m_mapNPC.clear();
