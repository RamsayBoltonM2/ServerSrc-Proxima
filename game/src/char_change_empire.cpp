...
bool CHARACTER::CanChangeEmpireOfflineShopCheck()
{
	...
	std::unique_ptr<SQLMsg> pPidMsg(DBManager::instance().DirectQuery(szQuery));
...
#ifdef WJ_PLAYER_PER_ACCOUNT5
	snprintf(szQuery, sizeof(szQuery),
		"SELECT COUNT(*) FROM player.offline_shop_npc%s WHERE owner_id=%u OR owner_id=%u OR owner_id=%u OR owner_id=%u OR owner_id=%u",
		get_table_postfix(), dwPID[0], dwPID[1], dwPID[2], dwPID[3], dwPID[4]);
#else
	snprintf(szQuery, sizeof(szQuery),
		"SELECT COUNT(*) FROM player.offline_shop_npc%s WHERE owner_id=%u OR owner_id=%u OR owner_id=%u OR owner_id=%u",
