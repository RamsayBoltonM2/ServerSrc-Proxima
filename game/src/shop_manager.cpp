...
bool CShopManager::Initialize(TShopTable * table, int size)
{
	...
	for (i = 0; i < size; ++i, ++table)
	{
		...
		if (!shop->Create(table->dwVnum, table->dwNPCVnum, table->items, table->price_type, table->szShopName, table->bSize))
#else
...
void CShopManager::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	if (0 != g_BuySellTimeLimitValue)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("EVENT_FLAG_WAIT"));
			return;
		}
	}
