...
#if defined(WJ_WOLFMAN_CHARACTER) && defined(USE_LYCAN_CREATE_POSITION)
...
void CInputLogin::CharacterDelete(LPDESC d, const char * data)
{
	...
	snprintf(szQuery, sizeof(szQuery), "SELECT COUNT(*) FROM player.offline_shop_npc%s WHERE owner_id=%u", get_table_postfix(), c_rAccountTable.players[pinfo->index].dwID);
	std::unique_ptr<SQLMsg> pResMsg(DBManager::instance().DirectQuery(szQuery));
...
/*#ifdef WJ_MOUNT_SYSTEM
	...
	LPITEM costumeAcce = ch->GetWear(WEAR_ACCE);
	if (costumeAcce != nullptr)
	{
		if (costumeAcce->GetSocket(0) > 18)
