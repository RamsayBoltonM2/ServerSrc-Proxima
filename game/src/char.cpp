...
void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable * pTable, BYTE bItemCount, BYTE bNpcType)
#else
...
#endif
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Cannot open myshop with security key activate"));
		return;
	}
#endif
...
#if defined(WJ_CHEQUE_SYSTEM) and defined(WJ_MULTIPRICE_SYSTEM)
		if ((pTable+n)->price_type == 7)
			nTotalCheque += static_cast<int64_t>((pTable+n)->price2);
#elif defined(WJ_CHEQUE_SYSTEM) and !defined(WJ_MULTIPRICE_SYSTEM)
		nTotalCheque += static_cast<int64_t>((pTable+n)->price2);
#endif
	}
	...
	if (CHEQUE_MAX <= nTotalCheque)
	{
		...
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("PLAYER_MAX_WON"));
		return;
	}
#endif
	...
	for (BYTE i = 0; i < bItemCount; ++i)
	{
		...
		if (pkItem)
		{
			...
			// MYSHOP_PRICE_LIST
#ifdef WJ_CHEQUE_SYSTEM
			itemkind[pkItem->GetVnum()] = TItemPriceType((pTable + i)->price / pkItem->GetCount(), (pTable + i)->price2);
#else
	...
	if (bNpcType == 0)
		...
		SetPolymorph(30002, true);
	else if (bNpcType == 2)
		SetPolymorph(30003, true);
	else if (bNpcType == 3)
		SetPolymorph(30004, true);
#else
	SetPolymorph(30000, true);
#endif
}
...
#if defined(WJ_OFFLINESHOP_SYSTEM) and defined(WJ_GROWTH_PET_SYSTEM)
		if (IsPC() == true || IsOfflineShopNPC() == true || IsNewPet() == true)
#elif defined(WJ_OFFLINESHOP_SYSTEM) and !defined(WJ_GROWTH_PET_SYSTEM)
		if (IsPC() == true || IsOfflineShopNPC() == true)
#elif !defined(WJ_OFFLINESHOP_SYSTEM) and defined(WJ_GROWTH_PET_SYSTEM)
...
void CHARACTER::UpdatePacket()
{
	...
	if (IsPC() && (!GetDesc() || !GetDesc()->GetCharacter())) return;
	...
	pack.dwLevel = GetLevel();	// level update fix
...
void CHARACTER::CreatePlayerProto(TPlayerTable & tab)
{
	...
	tab.inventory_ex = GetInventoryEx();
#endif
#ifdef WJ_NATIONAL_POINT_SYSTEM
	tab.national_point = GetNationalPoint();
#endif
#ifdef WJ_GEM_SYSTEM
	tab.gaya 		= GetGaya();
#endif
...
#ifdef ENABLE_PCBANG_FEATURE // @warme2
	{
...
#ifdef WJ_INVENTORY_EX_SYSTEM
	pack.points[POINT_INVENTORY_EX]	= GetInventoryEx();
#endif
#ifdef WJ_CHEQUE_SYSTEM
	pack.points[POINT_CHEQUE]	= GetCheque();
#endif
#ifdef WJ_NATIONAL_POINT_SYSTEM
	pack.points[POINT_NATIONAL_POINT]	= GetNationalPoint();
#endif
#ifdef WJ_GEM_SYSTEM
...
#endif
#ifdef WJ_INVENTORY_EX_SYSTEM
	SetInventoryEx(t->inventory_ex);
#endif
#ifdef WJ_NATIONAL_POINT_SYSTEM
	SetNationalPoint(t->national_point);
#endif
#ifdef WJ_GEM_SYSTEM
	SetGaya(t->gaya);
#endif
...
DWORD CHARACTER::GetMotionMode() const
{
	...
	if ((pkItem = GetWear(WEAR_COSTUME_WEAPON)))
	{
		switch (pkItem->GetValue(3))
		{
			case WEAPON_SWORD:
				dwMode = MOTION_MODE_ONEHAND_SWORD;
				break;
...
int CHARACTER::GetPolymorphPoint(BYTE type) const
{
	if (IsPolymorphed() && !IsPolyMaintainStat())
	{
		...
		if (pMob)
		{
			...
			{
				case POINT_ST:
					if ((GetJob() == JOB_SHAMAN || GetJob() == JOB_SURA) && GetSkillGroup() == 2) // @fixme
...
#ifdef WJ_INVENTORY_EX_SYSTEM
		case POINT_INVENTORY_EX:
			{
				const int64_t nTotalInventoryEx = static_cast<int64_t>(GetInventoryEx()) + static_cast<int64_t>(amount);
				if (180 < nTotalInventoryEx)
				{
					sys_err("[OVERFLOW_INVENTORY] OriInventory %d AddedInventory %d id %u Name %s ", GetInventoryEx(), amount, GetPlayerID(), GetName());
					LogManager::instance().CharLog(this, GetInventoryEx() + amount, "OVERFLOW_INVENTORY", "");
					return;
				}
				SetInventoryEx(GetInventoryEx() + amount);
				val = GetInventoryEx();
			}
			break;
#endif
...
#ifdef WJ_SASH SYSTEM
		case POINT_SASHDRAIN_RATE:
#endif
...
#ifdef WJ_NATIONAL_POINT_SYSTEM
		case POINT_NATIONAL_POINT:
			{
				const long nTotalNationalPoint = static_cast<long>(GetNationalPoint()) + static_cast<long>(amount);
				
				if (NATIONAL_POINT_MAX <= nTotalNationalPoint)
				{
					sys_err("[OVERFLOW_NP] OriNP %d AddedNP %d id %u Name %s ", GetNationalPoint(), amount, GetPlayerID(), GetName());
					LogManager::instance().CharLog(this, GetNationalPoint() + amount, "OVERFLOW_NP", "");
					return;
				}
								
				SetNationalPoint(GetNationalPoint()+amount);
				val = GetNationalPoint();
			}
			break;
#endif
...
void CHARACTER::OpenMyOfflineShop(const char * c_pszSign, TOfflineShopItemTable * pTable, BYTE bItemCount, BYTE bNpcType)
{
	...
	LPOFFLINESHOP pkOfflineShop = COfflineShopManager::instance().CreateOfflineShop(npc, GetPlayerID(), pTable, bItemCount, GetMapIndex(), OFFLINE_SHOP_DEFAULT_TIME);
	if (!pkOfflineShop)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_NOT_OPEN"));
		return;
		...
		if (!CTargetManager::instance().GetTargetInfo(pkChrCauser->GetPlayerID(), TARGET_TYPE_VID, GetVID()))
		{
			...
			if (GetMyShop())
			{
				...
				if (pkChrCauser == this) // 자기는 가능
				{
					...
					if ((GetExchange() || IsOpenSafebox() || GetShopOwner()) || IsCubeOpen() || GetOfflineShopOwner() || IsAcceWindowOpen())
...
#elif !defined(WJ_OFFLINESHOP_SYSTEM) and defined(WJ_ACCE_SYSTEM)
					if ((GetExchange() || IsOpenSafebox() || GetShopOwner()) || IsCubeOpen() || IsAcceWindowOpen())
#else
...
#if defined(WJ_OFFLINESHOP_SYSTEM) and defined(WJ_ACCE_SYSTEM)
					if ((pkChrCauser->GetExchange() || pkChrCauser->IsOpenSafebox() || pkChrCauser->GetMyShop() || pkChrCauser->GetShopOwner()) || pkChrCauser->IsCubeOpen() || pkChrCauser->GetOfflineShopOwner() || pkChrCauser->IsAcceWindowOpen())
#elif defined(WJ_OFFLINESHOP_SYSTEM) and !defined(WJ_ACCE_SYSTEM)
...
#elif !defined(WJ_OFFLINESHOP_SYSTEM) and defined(WJ_ACCE_SYSTEM)
					if ((pkChrCauser->GetExchange() || pkChrCauser->IsOpenSafebox() || pkChrCauser->GetMyShop() || pkChrCauser->GetShopOwner()) || pkChrCauser->IsCubeOpen() || pkChrCauser->IsAcceWindowOpen())
#else
...
void CHARACTER::SetTarget(LPCHARACTER pkChrTarget)
{
	...
	if (m_pkChrTarget)
	{
		...
		else
		{
			...
			{
				if (m_pkChrTarget->GetMaxHP() <= 0) // @fixme8
					p.bHPPercent = 0;
				else
...
void CHARACTER::MountSummon(bool bSummon, int mount_vnum, const char * pHorseName)
{
	if ( bSummon )
	{
		...
		char buf[64];
		snprintf(buf, sizeof(buf), "%s - %s", GetName(), m_chMount->GetMobTable().szLocaleName);
		m_chMount->m_stName = buf;
...
{
	if (GetRaceNum() >= 20201 && GetRaceNum() <= 20251)
		return true;
...
void CHARACTER::SetBlockMode(int bFlag)
{
	...
	SetQuestFlag("game_option.block_pvp", bFlag & BLOCK_PVP ? 1 : 0);
}
...
#if defined(WJ_WOLFMAN_CHARACTER) && !defined(WJ_MAGIC_REDUCION_BONUS)
	const int METIN_STONE_NUM = 16;
#elif defined(WJ_WOLFMAN_CHARACTER) && defined(WJ_MAGIC_REDUCION_BONUS)
...
void CHARACTER::MountVnum(DWORD vnum)
{
	...
	if ((m_dwMountVnum != 0)&&(vnum!=0)) //@fixme9
		MountVnum(0);
...
bool CHARACTER::WarpToPID(DWORD dwPID)
{
	...
	if ((victim = (CHARACTER_MANAGER::instance().FindByPID(dwPID))))
	{
		...
		if (IS_SUMMONABLE_ZONE(mapIdx))
		{
			...
			{
				WarpSet(victim->GetX(), victim->GetY(), mapIdx); // @fixme
			}
			else
			{
...
bool CHARACTER::IsHack(bool bSendMsg, bool bCheckShopOwner, int limittime)
{
	...
	if (bCheckShopOwner)
	{
		...
		{
			if (bSendMsg)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("상대방이 다른 거래를 하고 있는 중입니다."));
			return true;						
		}
#endif
	}
...
#ifdef WJ_NATIONAL_POINT_SYSTEM
long CHARACTER::GetNationalPointByVictimLevel()
{
	if (GetLevel() >= 30 && GetLevel() <= 69)
		return 5;
	else if (GetLevel() >= 70 && GetLevel() <= 84)
		return 7;
	else if (GetLevel() >= 85 && GetLevel() <= 94)
		return 10;
	else if (GetLevel() >= 95 && GetLevel() <= 105)
		return 15;
	else
		return 0;
}
...
void CHARACTER::AddClMaterial(TItemPos tPos, BYTE bPos)
{
	...
	{
		bool bStop = false;
		if ((pkItem->GetType() == ITEM_COSTUME)  && (pkItemMaterial[0]->GetType() == ITEM_COSTUME))
		{
			if ((pkItem->GetSubType() == COSTUME_HAIR) && (pkItemMaterial[0]->GetSubType() != COSTUME_HAIR))
				bStop = true;
			else if ((pkItem->GetSubType() == COSTUME_BODY) && (pkItemMaterial[0]->GetSubType() != COSTUME_BODY))
				bStop = true;
			else if ((pkItem->GetSubType() == COSTUME_WEAPON) && (pkItemMaterial[0]->GetSubType() != COSTUME_WEAPON))
				bStop = true;
		}
		else if ((pkItem->GetType() == ITEM_COSTUME) && (pkItemMaterial[0]->GetType() == ITEM_WEAPON))
		{
			if ((pkItem->GetSubType() == COSTUME_WEAPON))
			{
				if ((pkItem->GetValue(3) != pkItemMaterial[0]->GetSubType()))
					bStop = true;
			}
			else
			{
		...
		else if ((pkItem->GetType() == ITEM_COSTUME) && (pkItemMaterial[0]->GetType() == ITEM_ARMOR))
		{
			if ((pkItem->GetSubType() != COSTUME_BODY))
				bStop = true;
		}
		else if ((pkItem->GetType() == ITEM_WEAPON) && (pkItemMaterial[0]->GetType() == ITEM_COSTUME))
		{
			if ((pkItemMaterial[0]->GetType() == ITEM_COSTUME))
			{
