...
namespace TempleOchao
{
	...
	void CMgr::Initialize()
	{
		...
		s_pkGuardianKilled = NULL;
		s_pkGuardianActivity = NULL;
	}
	...
	void CMgr::Destroy()
	{
		...
		if (s_pkGuardianKilled != NULL)
			event_cancel(&s_pkGuardianKilled);
		
		if (s_pkGuardianActivity != NULL)
	...
	void CMgr::Spawn()
	{
		...
		if ((oldEnTai != NULL) && (oldEnTai->GetMapIndex() == TEMPLE_OCHAO_MAP_INDEX))
	...
	{
		LPCHARACTER oldEnTai = CHARACTER_MANAGER::instance().Find(GetGuardianVID());
		if ((oldEnTai != NULL) && (oldEnTai->GetVictim()) && (oldEnTai->GetMapIndex() == TEMPLE_OCHAO_MAP_INDEX))
			return;
