...
ACMD(do_war)
{
	...
	DWORD type = GUILD_WAR_TYPE_FIELD; //fixme20
	two_arguments(argument, arg1, sizeof(arg1), arg2, sizeof(arg2));
...
#ifdef ENABLE_MONARCH_MOB_CMD_MAP_CHECK // @warme2
...
ACMD(do_PetEvo)
{
	...
	if (ch->GetNewPetSystem()->IsActivePet())
	{
		...
		if (tmpevo == 2 && ch->GetNewPetSystem()->GetLevel() == 81 && ch->GetNewPetSystem()->GetAge() < 30)      
		{
            ch->ChatPacket(CHAT_TYPE_INFO, "PET_AGE_HAVETO_MIN_30");
            return;
        }
			...
			for (int b = 0; b < 1; b++)
			{
				if (ch->CountSpecifyItem(it[tmpevo][b]) < ic[tmpevo][b])
				{
					...
					for (int c = 0; c < 1; c++)
					{
						...
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("PET_ITEMS_REQUIRED_RESULT"), ITEM_MANAGER::instance().GetTable(vnum)->szLocaleName, ic[tmpevo][c]);
					}
...
ACMD(do_IncreasePetSkill)
{
	...
	if (!item)
		return;
	int skill = item->GetVnum()-55009;
	CNewPetSystem* petSystem = ch->GetNewPetSystem();
	if (!petSystem)
		return;
	if (item->GetVnum() < 55009 && item->GetVnum() > 55027)
	{
...
ACMD(do_input_security)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("SQLINJECTERROR"));
		return;		
	}
...
ACMD(do_enchant2)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("속성을 변경하였습니다."));
	{
		char buf[128];
		snprintf(buf, sizeof(buf), "%u", itemTarget->GetID());
