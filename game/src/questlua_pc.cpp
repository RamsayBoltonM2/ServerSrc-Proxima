...
namespace quest 
{
	...
	int pc_give_cheque(lua_State* L)
	{
		...
		DBManager::instance().SendChequeLog(CHEQUE_LOG_QUEST, ch->GetPlayerID(), iAmount);
	...
	int pc_change_cheque(lua_State * L)
	{
		...
		{
			DBManager::instance().SendChequeLog(CHEQUE_LOG_QUEST, ch->GetPlayerID(), cheque);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	int pc_get_national_point(lua_State* L)
	{
		lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetNationalPoint());
		return 1;
	}
	
	int pc_change_national_point(lua_State * L)
	{
		long np = (long)lua_tonumber(L, -1);

		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		if (np + ch->GetNationalPoint() < 0)
			sys_err("QUEST wrong ChangeNationalPoint %d (now %d)", np, ch->GetNationalPoint());
		else
			ch->PointChange(POINT_NATIONAL_POINT, np, true);
	...
	int pc_acce_open_combine(lua_State* L)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ACCE_WINDOW_ALREADY_OPEN"));
		}
		return 1;
	}
...
#ifdef WJ_NATIONAL_POINT_SYSTEM
			{ "get_national_point",	pc_get_national_point		},
			{ "change_national_point",	pc_change_national_point	},
#endif			
#ifdef WJ_OKEY_CARD_EVENT
