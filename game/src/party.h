...
struct FPartyDropDiceRoll
{
	...
	void Process(const LPCHARACTER mobVictim)
	{
		if ((!mobVictim || (mobVictim->GetMobRank() >= MOB_RANK_BOSS && mobVictim->GetMobRank() <= MOB_RANK_KING)) && m_itemOwner->GetParty() && m_itemOwner->GetParty()->GetNearMemberCount() > 1)
		{
			...
			pParty->ChatPacketToAllMember(CHAT_TYPE_DICE_INFO_NORM, LC_TEXT("*** Rolling for the following item: %16s ***"), m_itemDrop->GetName());

			pParty->ForEachNearMember(*this);
			if (m_itemOwner)
			{
				m_itemDrop->SetOwnership(m_itemOwner);
				pParty->ChatPacketToAllMember(CHAT_TYPE_DICE_INFO_COLOR, LC_TEXT("*** Rolling of %s: %16s ***"), m_itemDrop->GetName(), m_itemOwner->GetName());
			}
		}
		else
			m_itemDrop->SetOwnership(m_itemOwner);
	...
	void operator () (LPCHARACTER ch)
	{
		...
		while (true)
		{
			...
			pParty->ChatPacketToAllMember(CHAT_TYPE_DICE_INFO_NORM, LC_TEXT("*** -> %16s - Dice score: %05d ***"), ch->GetName(), pickedNumber);
			break;
		}
	}
};
