...
void DBManager::LoginPrepare(BYTE bBillType, DWORD dwBillID, long lRemainSecs, LPDESC d, DWORD * pdwClientKey, int * paiPremiumTimes)
{
	...
	{
#ifdef ENABLE_PASSPOD_FEATURE // @warme2
		{
			if (!g_bNoPasspod)
			{
		...
		case QID_AUTH_LOGIN:
			{
				...
				if (pMsg->Get()->uiNumRows == 0)
				{
#ifdef ENABLE_BRAZIL_AUTH_FEATURE // @warme2
					{
						// 계정이 없으면 새로 만들어야 한다
		...
		case QID_BRAZIL_CREATE_ID :
			{
				...
				else
				{
					...
					// @fixme 1. PASSWORD('%s') -> %s 2. pinfo->passwd wrapped inside mysql_hash_password(%s).c_str()
...
void DBManager::SendChequeLog(BYTE type, DWORD vnum, int cheque)
{
	if (!cheque)
		return;
	TPacketChequeLog p;
	...
	p.vnum = vnum;
	p.cheque = cheque;
	db_clientdesc->DBPacket(HEADER_GD_CHEQUE_LOG, 0, &p, sizeof(p));
}
#endif
