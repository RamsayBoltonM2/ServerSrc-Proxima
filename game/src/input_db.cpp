		...
		while (NULL != (row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
		{
			...
			if (g_bChannel == bChannel)
			{
				...
				if (npc)
				{
					...
					if (iTime != 0)
					{
						DBManager::Instance().DirectQuery("UPDATE player.player SET offshop_time=%d WHERE id=%u", time(0)+iTime, dwOwnerID);
						npc->SetOfflineShopTimer(iTime);
						npc->StartOfflineShopUpdateEvent();
					}
					// end of bug fixed - 6.20.2015
										
					LPOFFLINESHOP pkOfflineShop = COfflineShopManager::instance().CreateOfflineShop(npc, dwOwnerID, NULL, 0, lMapIndex, iTime, szSign);
...
void CInputDB::ChequeLog(const char* c_pData)
{
	TPacketChequeLog * p = (TPacketChequeLog *) c_pData;
	
	if (g_bAuthServer == true)
		return;
	
	LogManager::instance().ChequeLog(p->type, p->vnum, p->cheque);
}
#endif
