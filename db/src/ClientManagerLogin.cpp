		...
		for (j = 0; j < PLAYER_PER_ACCOUNT; ++j)
		{
			if (pkTab->players[j].dwID == player_id)
			{
...
#ifdef WJ_NATIONAL_POINT_SYSTEM
					pkTab->players[j].lNationalPoint	= pt->national_point;
#endif
				...
				else
				{
					...
					pkTab->players[j].bChangeName	= 0;
#ifdef WJ_NATIONAL_POINT_SYSTEM
					pkTab->players[j].lNationalPoint = 0;
#endif
					pkTab->players[j].last_play = 0;
#ifdef WJ_ACCE_SYSTEM
...
#ifdef WJ_NATIONAL_POINT_SYSTEM
					str_to_number(pkTab->players[j].lNationalPoint, row[col++]);
#endif
					/*long retValue = 0;
					str_to_number(retValue, row[col++]);
					time_t last_play = retValue;
					struct tm * tm1;
					tm1 = localtime(&last_play);*/
