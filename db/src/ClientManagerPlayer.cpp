...
char* CClientManager::GetCommand(char* str, char* command) // @fixme2
{
	char* tok;
...
#ifdef WJ_INVENTORY_EX_SYSTEM
	str_to_number(pkTab->inventory_ex, row[col++]);
#endif
#ifdef WJ_CHEQUE_SYSTEM
	str_to_number(pkTab->cheque, row[col++]);
#endif
#ifdef WJ_NATIONAL_POINT_SYSTEM
	str_to_number(pkTab->national_point, row[col++]);
#endif
#ifdef WJ_GEM_SYSTEM
