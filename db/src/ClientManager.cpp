				...
				while (it != pSet->end())
				{
...

					{
						...
						snprintf(szQuery, sizeof(szQuery), 
								...
								iPos,
								pItemAward->dwVnum, pItemAward->dwCount, pItemAward->dwSocket0, pItemAward->dwSocket1, dwSocket2, pItemAward->dwSocket3, pItemAward->dwSocket4, pItemAward->dwSocket5);
					}
...
#ifdef WJ_CHEQUE_SYSTEM
void CClientManager::ChequeLog(TPacketChequeLog* p)
{
	CChequeLog::instance().AddLog(p->type, p->vnum, p->cheque);
}
#endif
...
//
void CClientManager::MyshopPricelistUpdate(const TItemPriceListTable* pPacket) // @fixme (TPacketMyshopPricelistHeader to TItemPriceListTable)
{
	...
	while (peer->PeekPacket(i, header, dwHandle, dwLength, &data))
	{
		...
		switch (header)
		{
...
#ifdef WJ_CHEQUE_SYSTEM
			case HEADER_GD_CHEQUE_LOG:
				ChequeLog((TPacketChequeLog*)data);
				break;
#endif
			...
			case HEADER_GD_MYSHOP_PRICELIST_UPDATE:
				MyshopPricelistUpdate((TItemPriceListTable*)data); // @fixme (TPacketMyshopPricelistHeader to TItemPriceListTable)
				break;
